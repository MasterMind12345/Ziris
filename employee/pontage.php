...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'debut' && !$presence_aujourdhui) {
            ...
            // Utiliser l'heure du client si disponible et valide
            if (!empty($client_time)) {
                $heure_debut = date('H:i:s', strtotime($client_time));
                
                // Validation : l'heure ne peut pas être dans le futur de plus de 5 minutes
                $server_time = time();
                $client_timestamp = strtotime($client_time);
                $time_difference = $client_timestamp - $server_time;
                
                if ($time_difference > 300) { // 5 minutes dans le futur
            ...
            $stmt->execute([$user_id, $heure_debut, $lieu, $retard_minutes, $latitude, $longitude]);
            ...
            // Utiliser l'heure du client si disponible et valide
            if (!empty($client_time)) {
                $heure_fin = date('H:i:s', strtotime($client_time));
                
                // Validation : l'heure ne peut pas être avant l'heure de début
                $heure_debut_timestamp = strtotime($presence_aujourdhui['heure_debut_reel']);
                $heure_fin_timestamp = strtotime($heure_fin);
                
                if ($heure_fin_timestamp <= $heure_debut_timestamp) {
                    $heure_fin = date('H:i:s');
                }
                
                // Validation : l'heure ne peut pas être dans le futur de plus de 5 minutes
                $server_time = time();
                $time_difference = $heure_fin_timestamp - $server_time;
            ...
            $latitude_fin = $_POST['latitude'] ?? null;
            $longitude_fin = $_POST['longitude'] ?? null;
            $lieu_fin = "Bureau";
            if (isset($_POST['latitude']) && isset($_POST['longitude'])) {
                $lieu_fin = "Localisation: " . $_POST['latitude'] . ", " . $_POST['longitude'];
            }
            
            $stmt = $pdo->prepare("UPDATE presences SET heure_fin_reel = ?, lieu_fin = ?, latitude_fin = ?, longitude_fin = ? WHERE id = ?");
            $stmt->execute([$heure_fin, $lieu_fin, $latitude_fin, $longitude_fin, $presence_aujourdhui['id']]);
...
<body>
    ...
    <main class="employee-main">
        <div class="page-header">
            ...
            <p>Enregistrez votre arrivée et départ avec l'heure locale exacte</p>
        </div>
                ...
                <div class="actions-content">
                    <?php if (!$presence_aujourdhui): ?>
                        <form method="POST" id="pointageForm">
                            ...
                            <div class="action-info">
                                ...
                                <p>Pointage d'arrivée avec heure locale exacte</p>
                                ...
                                <div class="heure-info">
                                    <small>Heure locale détectée: <span id="localTime">Chargement...</span></small>
                                </div>
                    ...
                    <?php elseif (!$presence_aujourdhui['heure_fin_reel']): ?>
                        <form method="POST" id="pointageFinForm">
                            ...
                            <input type="hidden" name="latitude" id="latitude_fin">
                            <input type="hidden" name="longitude" id="longitude_fin">
                            <input type="hidden" name="client_time" id="client_time_fin">
                            
                            <div class="action-info">
                                <h3>Terminer la Journée</h3>
                                <p>Pointage de départ avec heure locale exacte</p>
                                
                                <div class="heure-info">
                                    <small>Heure locale détectée: <span id="localTimeFin">Chargement...</span></small>
                                </div>
                            ...
                            <button type="submit" class="btn-action end" id="btnEnd">
            ...
            <div class="info-content">
                <div class="info-item">
                    ...
                    <div>
                        ...
                        <p>Votre heure locale est automatiquement détectée et utilisée pour le pointage, garantissant une précision totale.</p>
                    </div>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <div>
                        ...
                        <p>Le système valide automatiquement l'heure locale pour éviter toute manipulation ou erreur de fuseau horaire.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <style>
        /* CSS pour l'interface employé (identique à celui que vous avez fourni) */
        :root {
            --primary: #4361ee;
            --primary-dark: #3a56d4;
            --secondary: #7209b7;
    ...
    <script>
        // Fonction pour obtenir l'heure locale du client au format ISO
        function getClientLocalTime() {
            const now = new Date();
            return now.toISOString(); // Format: "2025-11-30T19:12:54.123Z"
        }
        ...
        function getLocation() {
            ...
            // Mettre à jour l'heure locale affichée
            localTimeElement.textContent = formatLocalTimeForDisplay();
            ...
            if (navigator.geolocation) {
                ...
                navigator.geolocation.getCurrentPosition(
                    function(position) {
                        ...
                        // Mettre à jour l'heure client juste avant l'envoi
                        document.getElementById('client_time').value = getClientLocalTime();
                    ...
                    function(error) {
                        ...
                        // Mettre à jour l'heure client même sans géolocalisation
                        document.getElementById('client_time').value = getClientLocalTime();
                        btnStart.disabled = false;
                    },
                    {
        ...
        // Fonction pour le pointage de fin
        function setupFinPointage() {
            const localTimeElementFin = document.getElementById('localTimeFin');
            const btnEnd = document.getElementById('btnEnd');
            const formFin = document.getElementById('pointageFinForm');
            
            // Mettre à jour l'heure locale affichée
            localTimeElementFin.textContent = formatLocalTimeForDisplay();
            
            // Préparer l'heure client pour le formulaire de fin
            if (formFin) {
                formFin.addEventListener('submit', function(e) {
                    // Mettre à jour l'heure client juste avant l'envoi
                    document.getElementById('client_time_fin').value = getClientLocalTime();
                    
                    // Obtenir la géolocalisation si disponible
                    if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                            function(position) {
                                document.getElementById('latitude_fin').value = position.coords.latitude;
                                document.getElementById('longitude_fin').value = position.coords.longitude;
                                formFin.submit();
                            },
                            function(error) {
                                console.error('Erreur de géolocalisation pour le départ:', error);
                                formFin.submit();
                            },
                            {
                                enableHighAccuracy: true,
        ...
        function checkWorkLocation(lat, lng) {
            ...
            const workLat = 4.05340160; // Douala par défaut (selon votre base de données)
            const workLng = 9.73537280;
        ...
        // Mettre à jour l'heure locale en temps réel pour l'affichage
